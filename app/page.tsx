...
export default async function Home({
  ...
  const users: MaidanUser[] = JSON.parse(
    await fs.readFile(process.cwd() + "/app/data/users.json", "utf-8")
  );
  const count: MaidanCount = JSON.parse(
    await fs.readFile(process.cwd() + "/app/data/count.json", "utf-8")
  );
  const orgs: MaidanOrganization[] = JSON.parse(
    await fs.readFile(process.cwd() + "/app/data/orgs.json", "utf-8")
  );
